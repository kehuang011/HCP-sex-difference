...
#mask specific yeo<->yeo pairs
for iy in range(1,yeonum+1):
    for jy in range(iy,yeonum+1):
        if do_exclude_mask:
            yeopair_index_list.append((iy,jy))
            yeopair_name_list.append('EXCLUDE:%s<->%s' % (yeonames[iy-1],yeonames[jy-1]))
        if do_include_mask:
            yeopair_index_list.append((iy,jy))
            yeopair_name_list.append('INCLUDE:%s<->%s' % (yeonames[iy-1],yeonames[jy-1]))
...
for conntype in conntypes:
    ...
    for ypidx, (iy,jy) in enumerate(yeopair_index_list):
        if (iy,jy)==(-1,-1):
            ...
            m=((yeo1==iy) & (yeo2==jy)) | ((yeo1==jy) & (yeo2==iy))
...
if is_meanfit_shift:
    with open('/home/out_log/results_HCPdata_Kraken/newYA95/sensitivity/results1110/sensitivity_' + str(model_kraken) + '_' + str(bins) + '_stratifiedCV_split0328_test'+ str(testsize) + '_add' + str(add_trainingYA)+ '_downsampledtrain'+ str(train_downsample_size*2) + '_nwpair_meanshift.pkl', 'wb') as pickle_file:
        pickle.dump(masked_results, pickle_file)    
else:
    with open('/home/out_log/results_HCPdata_Kraken/newYA95/sensitivity/results1110/sensitivity_' + str(model_kraken) + '_' + str(bins) + '_stratifiedCV_split0328_test'+ str(testsize) + '_add' + str(add_trainingYA)+ '_downsampledtrain'+ str(train_downsample_size*2) + '_nwpair.pkl', 'wb') as pickle_file:
        pickle.dump(masked_results, pickle_file)    
