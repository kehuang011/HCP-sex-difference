...
def run_model_on_new_data(argv=None):
    ...
    conntype_conditional_name_list_dict={k:[] for k in conn_names}
    if 'conditional_name_list' in checkpoint['training_params']:
        for iconn, conn in enumerate(conn_names):
            conntype_conditional_name_list_dict[conn]=checkpoint['training_params']['conditional_name_list'][iconn]
    ...
    ############
    #breakpoint()
    #HACK to test fusion weights
    fusion_weights=None
    #computed previously:
    #fusion_weights=np.array([0.6357004046, 0.0002622020, 0.0009083830, 0.0002602387, 0.0000376561, 0.0000879886, 0.0018888108, 0.3587636948, 0.0009879838, 0.0002123518, 0.0001362388, 0.0001869944, 0.0001509695, 0.0001566406, 0.0002594435])
    #fusion_weights=np.array([ 0.2602559328, 0.0111263040, 0.0888580382, 0.0893610865, 0.0115369689, 0.0797559246, 0.0093508698, 0.2097434700, 0.1774005443, 0.0112959705, 0.0103664827, 0.0110419476, 0.0098616090, 0.0077011795, 0.0123437308 ])
    #these were from the batchwise test:
    #fusion_weights=np.array([ 0.0000548245, 0.0000542475, 0.1060180590, 0.0038520698, 0.0000429491, 0.0000545774, 0.0000538201, 0.4574917555, 0.1718764603, 0.0000486159, 0.0000567299, 0.0000551470, 0.1979766488, 0.0623251274, 0.0000389550 ])
    
    if fusion_weights is not None:
        fusion_weights/=np.sum(fusion_weights)
        print(f"Using fusion weights [", " ".join([f"{v:.3f}" for v in fusion_weights]),"]")
        for iconn, conn in enumerate(conn_names):
            print(f"{fusion_weights[iconn]:20.10f} {conn}")
        encoded_alltypes['fusion']=np.add.reduce([fusion_weights[iconn]*encoded_alltypes[conn]for iconn, conn in enumerate(conn_names)])
    ...
    for intype in encoded_alltypes.keys():
        ...
        for outtype in outtypes_for_this_input:
            ...
            conditional_input_data=None
            if conntype_conditional_name_list_dict[outtype] != []:
                for cn in conntype_conditional_name_list_dict[outtype]:
                    conddata_origscale=conndata_alltypes[cn]['data']
                    conddata=torchfloat(transformer_list[cn].transform(conddata_origscale))
                    if conditional_input_data is None:
                        conditional_input_data=conddata
                    else:
                        conditional_input_data=torch.cat((conditional_input_data,conddata),dim=1)
            ...
            if do_save_transformed_inputs:
                ...
                with torch.no_grad():
                    _ , conn_predicted = net(conn_encoded, neg1_torch, decoder_index_torch, conditional_input=conditional_input_data)
