...
def argument_parse_runtraining(argv):
    ...
    input_arg_group.add_argument('--flavorconditional',action='store',dest='flavor_conditional', help='Specify name1=name2[+name3] to add othername as a condition for adversarial loss name1 (eg: site=age)',nargs='*')
...
def run_training_command(argv=None):
    ...
    output_conntype_conditional_names={}
    ...
    if args.flavor_conditional is not None:
        for wstr in args.flavor_conditional:
            if not "=" in wstr:
               raise ValueError("Invalid flavor conditional input name. Must be name=othername: %s" % (wstr))
            gname,gval=wstr.split("=")
            gval=gval.split("+")
            output_conntype_conditional_names[gname]=gval
    ...
    for training_params in training_params_list:
        ...
        for grouptype in input_pathgroups:
            ...
            if len(input_trainpath_pairs) > 0:
                ...
                for x in input_conn_name_list:
                    for y in input_conn_name_list:
                        ...
                        if xg is None or yg is None:
                            ...
                            pass
                        elif grouptype == 'SCFC2all':
                            #eg: skip demographics->connectivity. only do SCFC->SCFC and SCFC->demographics
                            if not(xg == 'SC' or xg == 'FC'):
                                continue
                        elif grouptype == 'SCFC2demo':
                            #eg: skip demographics->connectivity. only do SCFC->SCFC and SCFC->demographics
                            if not(xg == 'SC' or xg == 'FC') or not yg == 'demo':
                                continue
                        elif grouptype == 'SCFC':
                            pass
                ...
                #build list of types for each group
                conngroup_conntypes={}
                for x in input_conn_name_list:
                ...
                ########### experiment to try fusion(SC+FC) -> demo
                if False:
                    y='age'
                    #x=conngroup_conntypes['SCFC']
                    x='fusion(SCFC)'
                    trainpath_pairs+=[[x,y]]
                ############ end experiment
            ...
            for i,x in enumerate(conn_names):
                if x in output_conntype_conditional_names:
                    condlist=output_conntype_conditional_names[x]
                else:
                    condlist=[]
                input_conditional_name_list.append(condlist)
            ########
            
            trainpath_weights=[]
            for itp,(x,y) in enumerate(trainpath_pairs):    
                #xg=conndata_alltypes[x]['group'] #not used
                yg=conndata_alltypes[y]['group']
            ...
            if do_target_encoding or do_fixed_target_encoding:
                ...
                trainpath_list, data_optimscale, data_orig, data_transformer_info_list = generate_training_paths(conndata_alltypes_targetencoding, conn_names, subjects, subjidx_train, subjidx_val, 
                                                ...
                                                input_conditional_name_list=input_conditional_name_list)
