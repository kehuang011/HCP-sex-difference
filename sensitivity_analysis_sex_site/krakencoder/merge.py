...
def merge_models(net_and_checkpoint_dict_list, canonicalize_input_names=False, input_names=None):
    ...
    for iconn, conn_name in enumerate(input_name_list):
        ...
        if hasattr(net, 'conditional_size_list'):
            net.conditional_size_list.append(net_tmp.conditional_size_list[iinput])
        if hasattr(net, 'decoder_gradientreversal_list'):
            net.decoder_gradientreversal_list.append(net_tmp.decoder_gradientreversal_list[iinput])
    ...
    #need to merge these fields in training_params because they are accessed in run_model
    training_params_to_merge=['conditional_size_list','conditional_name_list']
    for k in training_params_to_merge:
        if not k in checkpoint_info['training_params']:
            continue
        v=checkpoint_info['training_params'][k]
        if len(v)==num_inputs_orig0:
            #this field is a list of values for each of the original input names.
            #we need to merge and reorder it across all checkpoints to match the new input names
            vnew=[]
            for iconn, conn_name in enumerate(input_name_list):
                ...
                iinput=input_name_list_source_input_idx[iconn]
                chk_tmp=net_and_checkpoint_dict_list[inet]['checkpoint']
                vnew.append(chk_tmp['training_params'][k][iinput])
            print("Merging input field checkpoint_info[training_params]['%s']: %d items" % (k,len(vnew)))
            checkpoint_info['training_params'][k]=vnew
    
    if hasattr(net, 'adversarial_boolean_list'):
        checkpoint_info['adversarial_boolean_list']=net.adversarial_boolean_list
    if hasattr(net, 'conditional_size_list'):
        checkpoint_info['conditional_size_list']=net.conditional_size_list
    if hasattr(net, 'decoder_gradientreversal_list'):
