...
class Krakencoder(nn.Module):
    ...
    def __init__(self,inputsize_list,latentsize=128,hiddenlayers=[],skip_relu=False, dropout=0, 
                 ...
                 adversarial_boolean_list=[],conditional_size_list=[]):
        """
        ...
        if len(conditional_size_list) == 0:
            conditional_size_list=[0]*len(inputsize_list)
        if len(conditional_size_list) != len(inputsize_list):
            raise ValueError("conditional_size_list must be same length as inputsize_list")
        self.adversarial_boolean_list=adversarial_boolean_list
        self.conditional_size_list=conditional_size_list
        ...
        for i_input,inputsize in enumerate(inputsize_list):
            ...
            if self.conditional_size_list[i_input] > 0:
                #add conditional size to decoder input
                dec_size_list[0]+=self.conditional_size_list[i_input]
    ...
    def forward(self, x, encoder_index, decoder_index, transcoder_index_list=None, conditional_input=None):
        """
        ...
        if self.intergroup_transform_list is not None and encoder_index >= 0 and decoder_index >= 0:
            ...
            if ijstr in self.intergroup_transform_index_dict:
                ...
                #don't save the intercode as x_enc_orig because we need that untouched for latentsim
                x_enc=self.intergroup_transform_list[ijidx](x_enc)
        
        if conditional_input is not None:
            #add conditional input to the decoder
            x_enc = torch.cat((x_enc,conditional_input),1)
    ...
    def save_checkpoint(self, filename, extra_dict=None):
        ...
        checkpoint['conditional_size_list']=self.conditional_size_list
        ############### end adversarial
    ...
    def load_checkpoint(filename, checkpoint_override=None, eval_mode=False):
        ...
        if not 'conditional_size_list' in checkpoint:
            checkpoint['conditional_size_list']=[]
        ...
        net=Krakencoder(checkpoint['input_size_list'],latentsize=checkpoint['latentsize'],hiddenlayers=checkpoint['hiddenlayers'],
            ...
            conditional_size_list=checkpoint['conditional_size_list'])
        ############### end intergroup 
