...
def split_conditional_output(conn_targets, conditional_size=0):
    #quick function for splitting conn_targets that have [output, conditional] data concatenated
    if conditional_size == 0  or conditional_size is None:
        return conn_targets, None
    else:
        return conn_targets[:,:-conditional_size], conn_targets[:,-conditional_size:]
...
def generate_training_paths(conndata_alltypes, conn_names, subjects, subjidx_train, subjidx_val, trainpath_pairs=[],trainpath_group_pairs=[], 
    ...
    if len(input_conditional_name_list)==0:
        input_conditional_name_list=[[]]*len(conn_names)
    
    #modify this later once we know how big the specific conditional data is
    input_conditional_size_list=[0]*len(conn_names)
    ...
    traindata_list={}
    valdata_list={}
    traindata_conditional_list={}
    valdata_conditional_list={}
    ...
    traindata_conditional_list={c:None for c in conn_names}
    valdata_conditional_list={c:None for c in conn_names}
    for iconn,conn_name in enumerate(conn_names):
        condition_names=input_conditional_name_list[iconn]
        ...
        for cond in condition_names:
            condsize+=traindata_list[cond].shape[1]
            if newtrain_cond is None:
                newtrain_cond=traindata_list[cond].clone()
                newval_cond=valdata_list[cond].clone()
            else:
                newtrain_cond=torch.cat((newtrain_cond,traindata_list[cond]),dim=1)
                newval_cond=torch.cat((newval_cond,valdata_list[cond]),dim=1)
        traindata_conditional_list[conn_name]=newtrain_cond
        valdata_conditional_list[conn_name]=newval_cond
        input_conditional_size_list[iconn]=condsize
    ######## end adversarial+conditional
    ...
    data_optimscale_list['traindata_conditional']={k:torchfloat(v) if v is not None else None for k,v in traindata_conditional_list.items()}
    data_optimscale_list['valdata_conditional']={k:torchfloat(v) if v is not None else None for k,v in valdata_conditional_list.items()}
    ...
    for c1,c2 in trainpath_pairs:
        if type(c1) == str:
            if 'fusion(' in c1:
                fusiongroup=c1.split('fusion(')[-1].split(')')[0]
                fusiongroup_conntypes=conngroup_conntypes[fusiongroup]
                i1=[i for i,x in enumerate(conn_names) if x in fusiongroup_conntypes]
        ...
        train_inputs_conditional=traindata_conditional_list[c2]
        val_inputs_conditional=valdata_conditional_list[c2]
        
        tp_is_adversarial=input_is_adversarial[i2]
        tp_conditional_size=input_conditional_size_list[i2]
        ...
        train_inputs_conditional=data_optimscale_list['traindata_conditional'][c2]
        val_inputs_conditional=data_optimscale_list['valdata_conditional'][c2]
        
        trainloader=None
        if create_data_loader:
            train_outputs_forloader=train_outputs
            if train_inputs_conditional is not None:
                train_outputs_forloader=torch.cat((train_outputs_forloader,train_inputs_conditional),dim=1)
            
            if train_encoded is None:
                trainset = data_utils.TensorDataset(train_inputs, train_outputs_forloader)
            else:
                train_encoded=torchfloat(train_encoded)
                trainset = data_utils.TensorDataset(train_inputs, train_outputs_forloader, train_encoded)
        ...
        trainpath_tmp['train_inputs_conditional']=train_inputs_conditional
        ...
        valloader=None
        if create_data_loader:
            val_outputs_forloader=val_outputs
            if val_inputs_conditional is not None:
                val_outputs_forloader=torch.cat((val_outputs_forloader,val_inputs_conditional),dim=1)
            if val_encoded is None:
                valset = data_utils.TensorDataset(val_inputs, val_outputs_forloader)
            else:
                val_encoded=torchfloat(val_encoded)
                valset = data_utils.TensorDataset(val_inputs, val_outputs_forloader, val_encoded)
        ...
        trainpath_tmp['val_inputs_conditional']=val_inputs_conditional
        if val_encoded is not None:
            trainpath_tmp['val_encoded']=val_encoded
        
        trainpath_tmp['trainpath_adversarial']=tp_is_adversarial
        trainpath_tmp['trainpath_conditional_size']=tp_conditional_size
        ...
        trainpath_tmp['conditional_size_list']=input_conditional_size_list
        trainpath_tmp['conditional_name_list']=input_conditional_name_list
...
def train_network(trainpath_list, training_params, net=None, data_optimscale_list=None, data_origscale_list=None, trainfig=None, 
    ...
    conditional_size_list=trainpath_list[0]['conditional_size_list']
    conditional_name_list=trainpath_list[0]['conditional_name_list']
    ...
    if net is None:
        if intergroup:
            ...
            net = Krakencoder(coencoder_size_list, latentsize=latentsize, 
                                ...
                                adversarial_boolean_list=adversarial_boolean_list,conditional_size_list=conditional_size_list)
            ############# end intregroup
        else:
            net = Krakencoder(coencoder_size_list, latentsize=latentsize, 
                                hiddenlayers=hiddenlayers, skip_relu=skip_relu, dropout=dropout,
    ...
    trainrecord['conditional_name_list']=conditional_name_list
    trainrecord['conditional_size_list']=conditional_size_list
    trainrecord['adversarial_boolean_list']=adversarial_boolean_list
    ...
    #after making the checkpoint copy, need to convert conditional_name_list to a list of strings
    # so it can be saved in the record.mat
    if 'conditional_name_list' in trainrecord:
        trainrecord['conditional_name_list']=["+".join(cn) for cn in trainrecord['conditional_name_list']]
    ...
    for epoch in range(nbepochs):
        ...
        for ibatch_outer in range(batchloop_outerloopcount):
            ...
            for itp in trainpath_order:
                ...
                conditional_size_tp=trainpath['trainpath_conditional_size']
                ...
                train_inputs_conditional=trainpath['train_inputs_conditional']
                val_inputs_conditional=trainpath['val_inputs_conditional']
                ...
                for iloop in range(trainloops):
                    ...
                    for batch_idx in range(len(trainloader_iter)):
                        ...
                        if do_target_encoding:
                            ...
                            conn_targets, conn_targets_conditional = split_conditional_output(conn_targets, conditional_size_tp)
                            ...
                            if do_fixed_target_encoding:
                                ...
                                optimizer.zero_grad(set_to_none=do_zerograd_none)
                                _ , conn_predicted = net(conn_encoded_targets, neg1_index, decoder_index_torch, conditional_input=conn_targets_conditional)
                                ...
                                conn_encoded, conn_predicted = net(conn_inputs,encoder_index_torch,decoder_index_torch, conditional_input=conn_targets_conditional)
                            ...
                            conn_encoded, conn_predicted = net(conn_inputs,encoder_index_torch,decoder_index_torch, transcoder_index_list=transcoder_list, conditional_input=conn_targets_conditional)
        ...
        if do_compute_performance_AFTER_latentsim:
            ...
            for itp in trainpath_order:
                ...
                with torch.no_grad():
                    if do_fixed_target_encoding:
                        conn_encoded = net(train_inputs, encoder_index_torch, neg1_index)
                        _,conn_predicted = net(train_encoded, neg1_index, decoder_index_torch, conditional_input=train_inputs_conditional)
                    else:
                        ...
                        conn_encoded, conn_predicted = net(train_inputs, encoder_index_torch, decoder_index_torch, transcoder_index_list=transcoder_list, conditional_input=train_inputs_conditional)
                ...
                with torch.no_grad():
                    if do_fixed_target_encoding:
                        ...
                        _, conn_predicted = net(val_encoded, neg1_index, decoder_index_torch, conditional_input=val_inputs_conditional)
                    else:
                        conn_encoded, conn_predicted = net(val_inputs, encoder_index_torch, decoder_index_torch, transcoder_index_list=transcoder_list, conditional_input=val_inputs_conditional)
                #fc_preds = conn_predicted.cpu().detach().numpy()
