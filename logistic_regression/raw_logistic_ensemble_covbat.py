...
inputfile="%s/ls3_%s_1419subj.mat" % (datadir,contype) 
...
zeros_train = pd.DataFrame(0.0, index=constant_feats, columns=corrected_train.columns) # add dropped columns back as zeros
...
for i in range(len(bins)-1):
    ...
    for outer_iter in range(outer_folds):
        log(f"BIN {bins[i]}-{bins[i+1]}: Outer fold {outer_iter}")
        print(outer_iter)
        rng = seed + outer_iter
        np.random.seed(rng)
        ...
        X_train = train_df.iloc[:, 4:].to_numpy()
        X_test = test_df.iloc[:, 4:].to_numpy()
        y_train = train_df['sex_index'].to_numpy()
        y_test = test_df['sex_index'].to_numpy()
...
output_file = os.path.join(RESULTS_DIR, f'{contype}{bins}stratifiedCV_split0328_test{testsize}{add_trainingYA}_downsampledtrain{train_downsample_size*2}.mat')
