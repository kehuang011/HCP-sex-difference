    ...
    for i in range(len(bins)-1):
        ...
        if downsample == True:
            if dataname == 'krakenLS3dynICV403demofineSEX100_20251107_144040_ep000500_encoded': 
                subdir = 'krakencoder500_sex0508_permute/results1110'
            
            output_dir = os.path.join(RESULTS_DIR, subdir)
            os.makedirs(output_dir, exist_ok=True)
            output_file = os.path.join(output_dir, f'encoded_pred{type}{bins}_stratifiedCV_split0328_test{testsize}_add{add_trainingYA}oriYA{add_originalYA_per_sex}_downsampledtrain{train_downsample_size*2}_{method}_site.mat')
